...
*grid*='level'::
    ...
    (normal, tight, vtight/verytight, extreme are available).
...
*charges*='bool'::
    print charges file
*ptb dump*='bool'::
    write the PTB density matrix (ptb_density.npy/.npz), overlap matrix
    (ptb_overlap.npy/.npz) in the AO basis and the vDZP basis set in NWChem
    format (ptb_basis.nw) for real-space reconstruction; see the ptb group for
    the sparse output options (PTB only)
*mulliken*='bool'::
...
$ptb
~~~~
Options controlling the PTB density/overlap matrix export enabled by
'$write/ptb dump=true' (or *--ptbdump*).

*sparse*='bool'::
    write the density and overlap matrices as SciPy CSR sparse matrices
    (ptb_density.npz, ptb_overlap.npz) instead of dense NumPy arrays
    (default: false)
*threshold*='real'::
    when *sparse* is enabled, drop matrix elements with absolute value at or
    below this cutoff (default: 1.0e-8)
