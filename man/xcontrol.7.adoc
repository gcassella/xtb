...
*charges*='bool'::
    print charges file
*ptb dump*='bool'::
    write the PTB density matrix (ptb_density.npy/.npz).
*mulliken*='bool'::
...
*sparse*='bool'::
    write the density matrix in SciPy CSR sparse matrix form
    (ptb_density.npz) instead of dense NumPy arrays (default: false)
*threshold*='real'::
