...
*--ptbdump*::
    ...
    matrix ('ptb_overlap.npy') in the atomic-orbital basis, the vDZP basis set
    in NWChem format ('ptb_basis.nw'), and the atomic-orbital ordering of the
    matrices ('ptb_ao_order.npy'), together sufficient to reconstruct the
    real-space density. The matrices use tblite's spherical-harmonic component
    ordering (m = -l..+l per shell), which differs from the NWChem/PySCF
    ordering for p shells (px, py, pz); 'ptb_ao_order.npy' gives the (atom, l,
    m) of each row/column so a consumer can build the permutation to any target
