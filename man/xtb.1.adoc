...
*--ptbdump*::
    with *--ptb*, writes the density matrix ('ptb_density.npy'), in the atomic-
    orbital basis. The matrix is written in NumPy v1.0 array format by default;
    with 'sparse=true' in the '$ptb' control block it is written in SciPy
    CSR sparse matrix format ('ptb_density.npz') instead. The matrix uses
    tblite's spherical-harmonic component ordering (m = -l..+l per
