...
*--ptbdump*::
    ...
    the real-space density. The matrices use tblite's spherical-harmonic
    component ordering (m = -l..+l per shell), which differs from the
    NWChem/PySCF ordering for p shells (px, py, pz); a consumer must reorder
    the p rows and columns of the matrices accordingly.
